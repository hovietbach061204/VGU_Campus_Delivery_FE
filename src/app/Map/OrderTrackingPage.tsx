...
export default function OrderTrackingPage() {
  ...
  useEffect(() => {
    ...
    const fetchRouteAndEta = async () => {
      // Use GraphHopper API instead of Google Directions API
      const apiKey = '943be670-e612-4cc5-bb26-b43dfbc4cb2d';
      const url = `https://graphhopper.com/api/1/route?point=${deliveryman.lat},${deliveryman.lng}&point=${purchaser.lat},${purchaser.lng}&vehicle=car&locale=en&points_encoded=false&key=${apiKey}`;
      try {
        const res = await fetch(url);
        const data = await res.json();
        if (data.paths && data.paths[0]) {
          const points = data.paths[0].points.coordinates.map((coord: number[]) => ({ lat: coord[1], lng: coord[0] }));
          setRoute(points);
        ...
        >
          ...
          {purchaser && (
            <Marker position={purchaser}>
              <Popup>Purchaser Location</Popup>
            </Marker>
          )}
          {deliveryman && (
            <Marker position={deliveryman}>
              <Popup>Deliveryman Location</Popup>
            </Marker>
          )}
